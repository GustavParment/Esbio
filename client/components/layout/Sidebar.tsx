...
const navIcons: Record<string, React.ReactNode> = {
  ...
  invoices: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="18" rx="2"/><line x1="8" y1="7" x2="16" y2="7"/><line x1="8" y1="11" x2="16" y2="11"/><line x1="8" y1="15" x2="12" y2="15"/><path d="M17 15l2 2-2 2"/></svg>,
  ...
  bank: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21h18"/><path d="M3 10h18"/><path d="M5 6l7-3 7 3"/><path d="M4 10v11"/><path d="M20 10v11"/><path d="M8 14v3"/><path d="M12 14v3"/><path d="M16 14v3"/></svg>,
...
const navigation = [
  ...
  { name: "Fakturor", href: "/invoices", icon: "invoices" },
  { name: "Kunder", href: "/customers", icon: "customers" },
  { name: "Konton", href: "/accounts", icon: "accounts" },
  { name: "Bank", href: "/bank", icon: "bank" },
  ...
  { name: "Hjälp", href: "/help", icon: "help" },
];
  ...
  return (
    <>
      ...
      <div className={`
        ...
        flex h-full w-full lg:w-64 flex-col bg-[#0B1514]
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navigation
          .filter((item) => {
            // Mini plan: hide AI, invoicing, and bank tabs
            if (selectedCompany?.plan === "mini") {
              return item.href !== "/agent" && item.href !== "/invoices" && item.href !== "/customers" && item.href !== "/bank";
            }
            return true;
          })
          .map((item) => (
          ...
          >
            {item.icon === "help" ? (
              <span className="mr-3"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></span>
            ) : item.icon === "ester" ? (
        ...
        {/* Mobile-only: Dark mode + Logout (between nav and plan section) */}
        <div className="lg:hidden pt-4 mt-4 border-t border-[rgba(255,255,255,0.06)] space-y-1">
          <button
            onClick={toggleTheme}
      ...
      {/* Dark mode toggle (desktop only — mobile version is inside nav) */}
      ...
      {/* Logout (desktop only — mobile version is inside nav) */}
      <div className="hidden lg:block border-t border-[rgba(255,255,255,0.06)] px-3 py-4">
        <button
          onClick={() => { logout(); setMobileMenuOpen(false); }}
          className="flex items-center w-full px-3 py-2 text-sm font-medium rounded-md text-gray-300 hover:bg-red-900/30 hover:text-red-400 transition-colors"
        >
